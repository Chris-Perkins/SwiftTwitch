//
//  PostablePanelExtensionData.swift
//  NewTwitchAPIWrapper
//
...
/// `PostablePanelExtensionData` is a value-type that is used for the posting of a Panel Extension
/// data update to the New Twitch API.
public struct PostablePanelExtensionData {
    ...
    /// Initializes a `PostablePanelExtension` with the input parameters as properties
    ///
    /// - Parameters:
    ...
    /// Initializes a `PostablePanelExtension` with the input `receivedData` object to retrieve
    /// data from.
    ///
    /// - Parameter receivedData: The object to transcribe into a `PostablePanelExtension`
    /// - Throws: If the `id` or `version` of the input class was `nil`.
    public init(withReceivedData receivedData: ReceivedPanelExtensionData) throws {
